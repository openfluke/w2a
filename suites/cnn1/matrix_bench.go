...
func TimedMatrix() error {
	...
	fmt.Printf("\n  CNN1 timed matrix — FormatNone × %d dtypes × %d backends\n", len(core.AllDTypes), len(backends))
...
func TimedQuantMatrix() error {
	...
	fmt.Printf("\n  CNN1 timed quant matrix — %d formats × %d backends (Float32)\n", len(quant.AllFormats), len(backends))
	fmt.Printf("  shape batch=%d C=%d L=%d→%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.InChannels, cfg.SeqLen, cfg.OutLen(), simd.Enabled(), webgpu.Available())
...
func timeCell(dt core.DType, format quant.Format, be core.Backend, cfg cnn1.Config, batch, warm, iters int) (fwdNs, bwdNs int64, status, note string) {
	...
	for i := 0; i < warm; i++ {
		pre, _, err = cnn1.Forward(l, x)
		if err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
		if _, _, err = cnn1.Backward(l, gOut, x, pre); err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
	}
	if iters < 1 {
