...
func Cases() []Case {
	return []Case{
		{Name: "Forward smoke (same-pad identity-ish)", Run: forwardSmoke},
		{Name: "Output length / shape smoke", Run: shapeSmoke},
		...
		{Name: "§12 im2col quant WebGPU smoke (Q4_0)", Run: im2colQuantWebGPUSmoke},
...
func RunAll() error {
	...
	if len(fails) > 0 {
		return fmt.Errorf("cnn1: %d failed: %s", len(fails), strings.Join(fails, ", "))
	}
	return nil
}
...
func RunOne(n int) error {
	cs := Cases()
	if n < 1 || n > len(cs) {
		return fmt.Errorf("cnn1: case %d out of range 1..%d", n, len(cs))
	}
	suites.BeginCase()
	fmt.Printf("  [%d] %s … ", n, cs[n-1].Name)
	if err := cs[n-1].Run(); err != nil {
		suites.EndCase("cnn1", cs[n-1].Name, "FAIL", err.Error())
		fmt.Printf("FAIL\n      %v\n", err)
		return err
	}
	suites.EndCase("cnn1", cs[n-1].Name, "PASS", "")
	fmt.Println("PASS")
	return nil
}
...
func initWeights(cfg cnn1.Config) []float32 {
	n := cfg.Filters * cfg.InChannels * cfg.Kernel
	w := make([]float32, n)
	for i := range w {
		w[i] = float32((i%5)-2) * 0.05
	}
	// strengthen center tap on diagonal channels for a non-zero smoke signal
	for f := 0; f < cfg.Filters && f < cfg.InChannels; f++ {
		mid := f*cfg.InChannels*cfg.Kernel + f*cfg.Kernel + cfg.Kernel/2
		if mid < n {
			w[mid] = 1
		}
	}
...
func newLayer(cfg cnn1.Config, dt core.DType, format quant.Format, be core.Backend) (*cnn1.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	l, err := cnn1.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initWeights(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg cnn1.Config, batch int) *core.Tensor[float32] {
	x := core.NewTensor[float32](batch, cfg.InChannels, cfg.SeqLen)
	for i := range x.Data {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = cnn1.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = cnn1.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	fmt.Printf("(C=%d L=%d→%d) ", cfg.InChannels, cfg.SeqLen, cfg.OutLen())
	return nil
}
...
func shapeSmoke() error {
	...
	want := cfg.OutLen()
	if len(post.Shape) != 3 || post.Shape[0] != 2 || post.Shape[1] != 5 || post.Shape[2] != want {
		return fmt.Errorf("post shape %v want [2,5,%d]", post.Shape, want)
	}
	fmt.Printf("([2,5,%d]) ", want)
	return nil
}
...
func webGPUNoDevice() error {
	...
	_, _, err = cnn1.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
