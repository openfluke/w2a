...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  CNN1 TRAIN — %s\n", title)
	fmt.Printf("  cell CNN1 C=%d L=%d→%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  CNN1 TRAIN — %s\n", title)
	fmt.Printf("  cell CNN1 C=%d L=%d→%d batch=%d  cells=%d\n\n",
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg cnn1.Config) (ns int64, status, note string) {
	...
	g, err := buildCNN1Cube(n, be, dt, format, cfg)
	if err != nil {
...
func buildCNN1Cube(n int, be core.Backend, dt core.DType, format quant.Format, cfg cnn1.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if err := cnn1.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg cnn1.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
...
func trainBatch(batch int, cfg cnn1.Config) (x, y *core.Tensor[float32]) {
