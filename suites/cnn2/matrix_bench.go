...
func TimedMatrix() error {
	const batch, warm, iters = 2, 1, 4
	cfg := defaultCfg()
	backends := []core.Backend{core.BackendCPUTiled, core.BackendSIMD, core.BackendWebGPU}

	fmt.Printf("\n  CNN2 timed matrix — FormatNone × %d dtypes × %d backends\n", len(core.AllDTypes), len(backends))
	fmt.Printf("  shape batch=%d C=%d %dx%d→%dx%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.InChannels, cfg.Height, cfg.Width, cfg.OutH(), cfg.OutW(), simd.Enabled(), webgpu.Available())
...
func TimedQuantMatrix() error {
	...
	fmt.Printf("\n  CNN2 timed quant matrix — %d formats × %d backends (Float32)\n", len(quant.AllFormats), len(backends))
	fmt.Printf("  shape batch=%d C=%d %dx%d→%dx%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.InChannels, cfg.Height, cfg.Width, cfg.OutH(), cfg.OutW(), simd.Enabled(), webgpu.Available())
...
func timeCell(dt core.DType, format quant.Format, be core.Backend, cfg cnn2.Config, batch, warm, iters int) (fwdNs, bwdNs int64, status, note string) {
	...
	gOut := core.NewTensor[float32](batch, cfg.Filters, cfg.OutH(), cfg.OutW())
	for i := range gOut.Data {
		gOut.Data[i] = 1
	}
	var pre *core.Tensor[float32]
	for i := 0; i < warm; i++ {
		pre, _, err = cnn2.Forward(l, x)
		if err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
		if _, _, err = cnn2.Backward(l, gOut, x, pre); err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
	}
	if iters < 1 {
