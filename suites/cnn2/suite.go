...
func Cases() []Case {
	return []Case{
		{Name: "Forward smoke (same-pad 2D smoke)", Run: forwardSmoke},
...
func RunAll() error {
	...
	if len(fails) > 0 {
		return fmt.Errorf("cnn2: %d failed: %s", len(fails), strings.Join(fails, ", "))
	}
	return nil
}
...
func RunOne(n int) error {
	cs := Cases()
	if n < 1 || n > len(cs) {
		return fmt.Errorf("cnn2: case %d out of range 1..%d", n, len(cs))
	}
	suites.BeginCase()
	fmt.Printf("  [%d] %s … ", n, cs[n-1].Name)
	if err := cs[n-1].Run(); err != nil {
		suites.EndCase("cnn2", cs[n-1].Name, "FAIL", err.Error())
		fmt.Printf("FAIL\n      %v\n", err)
		return err
	}
	suites.EndCase("cnn2", cs[n-1].Name, "PASS", "")
	fmt.Println("PASS")
	return nil
}
...
func initWeights(cfg cnn2.Config) []float32 {
	n := cfg.Filters * cfg.InChannels * cfg.Kernel * cfg.Kernel
	w := make([]float32, n)
	for i := range w {
		w[i] = float32((i%5)-2) * 0.05
	}
	for f := 0; f < cfg.Filters && f < cfg.InChannels; f++ {
		mid := f*cfg.InChannels*cfg.Kernel*cfg.Kernel + f*cfg.Kernel*cfg.Kernel + (cfg.Kernel/2)*cfg.Kernel + cfg.Kernel/2
		if mid < n {
			w[mid] = 1
		}
	}
...
func newLayer(cfg cnn2.Config, dt core.DType, format quant.Format, be core.Backend) (*cnn2.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	l, err := cnn2.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initWeights(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg cnn2.Config, batch int) *core.Tensor[float32] {
	x := core.NewTensor[float32](batch, cfg.InChannels, cfg.Height, cfg.Width)
	for i := range x.Data {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = cnn2.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = cnn2.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	fmt.Printf("(C=%d %dx%d→%dx%d) ", cfg.InChannels, cfg.Height, cfg.Width, cfg.OutH(), cfg.OutW())
	return nil
}
...
func shapeSmoke() error {
	...
	oh, ow := cfg.OutH(), cfg.OutW()
	if len(post.Shape) != 4 || post.Shape[0] != 2 || post.Shape[1] != 5 || post.Shape[2] != oh || post.Shape[3] != ow {
		return fmt.Errorf("post shape %v want [2,5,%d,%d]", post.Shape, oh, ow)
	}
	fmt.Printf("([2,5,%d,%d]) ", oh, ow)
	return nil
}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := cnn2.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
...
func webGPUNoDevice() error {
	...
	_, _, err = cnn2.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
