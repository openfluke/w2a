...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  CNN2 TRAIN — %s\n", title)
	fmt.Printf("  cell CNN2 C=%d %dx%d→%dx%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  CNN2 TRAIN — %s\n", title)
	fmt.Printf("  cell CNN2 C=%d %dx%d→%dx%d batch=%d  cells=%d\n\n",
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg cnn2.Config) (ns int64, status, note string) {
	...
	g, err := buildCNN2Cube(n, be, dt, format, cfg)
	if err != nil {
...
func buildCNN2Cube(n int, be core.Backend, dt core.DType, format quant.Format, cfg cnn2.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if err := cnn2.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg cnn2.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
	...
	st, nt := suites.StampBackendNote("cnn2", be == core.BackendSIMD, be == core.BackendWebGPU, "OK", "")
	return total.Nanoseconds() / int64(iters), st, nt
}

func trainBatch(batch int, cfg cnn2.Config) (x, y *core.Tensor[float32]) {
