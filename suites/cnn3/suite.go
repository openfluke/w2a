...
func Cases() []Case {
	return []Case{
		{Name: "Forward smoke (same-pad 3D smoke)", Run: forwardSmoke},
		{Name: "Output spatial / shape smoke", Run: shapeSmoke},
		...
		{Name: "§12 tiled FormatNone f32 WebGPU↔CPU parity", Run: tiledWebGPUParity},
...
func RunAll() error {
	...
	if len(fails) > 0 {
		return fmt.Errorf("cnn3: %d failed: %s", len(fails), strings.Join(fails, ", "))
	}
	return nil
}
...
func RunOne(n int) error {
	cs := Cases()
	if n < 1 || n > len(cs) {
		return fmt.Errorf("cnn3: case %d out of range 1..%d", n, len(cs))
	}
	suites.BeginCase()
	fmt.Printf("  [%d] %s … ", n, cs[n-1].Name)
	if err := cs[n-1].Run(); err != nil {
		suites.EndCase("cnn3", cs[n-1].Name, "FAIL", err.Error())
		fmt.Printf("FAIL\n      %v\n", err)
		return err
	}
	suites.EndCase("cnn3", cs[n-1].Name, "PASS", "")
	fmt.Println("PASS")
	return nil
}

// shape-preserving for volumetric train: InCh==Filters, same-pad K=3 P=1
func tinyCfg() cnn3.Config {
	// PatchDim=64 + InCh==Filters for volumetric mesh chaining.
	return cnn3.Config{
		InChannels: 64, Filters: 64, Depth: 4, Height: 4, Width: 4, Kernel: 1, Stride: 1, Padding: 0,
		Activation: core.ActivationLinear,
	}
}

func defaultCfg() cnn3.Config {
	// PatchDim = InChannels*K³ = 8*2³ = 64 — AffinePacked packable.
	return cnn3.Config{
		InChannels: 8, Filters: 4, Depth: 6, Height: 6, Width: 6, Kernel: 2, Stride: 1, Padding: 1,
		Activation: core.ActivationLinear,
	}
}

func initWeights(cfg cnn3.Config) []float32 {
	k3 := cfg.Kernel * cfg.Kernel * cfg.Kernel
	n := cfg.Filters * cfg.InChannels * k3
	w := make([]float32, n)
	for i := range w {
		w[i] = float32((i%5)-2) * 0.05
	}
	for f := 0; f < cfg.Filters && f < cfg.InChannels; f++ {
		mid := f*cfg.InChannels*k3 + f*k3 + (cfg.Kernel/2)*cfg.Kernel*cfg.Kernel + (cfg.Kernel/2)*cfg.Kernel + cfg.Kernel/2
		if mid < n {
			w[mid] = 1
		}
	}
...
func newLayer(cfg cnn3.Config, dt core.DType, format quant.Format, be core.Backend) (*cnn3.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	l, err := cnn3.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initWeights(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg cnn3.Config, batch int) *core.Tensor[float32] {
	x := core.NewTensor[float32](batch, cfg.InChannels, cfg.Depth, cfg.Height, cfg.Width)
	for i := range x.Data {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = cnn3.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = cnn3.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	fmt.Printf("(C=%d %dx%dx%d→%dx%dx%d) ", cfg.InChannels, cfg.Depth, cfg.Height, cfg.Width, cfg.OutD(), cfg.OutH(), cfg.OutW())
	return nil
}

func shapeSmoke() error {
	cfg := tinyCfg()
	cfg.Filters = 5 // non-preserving ok for unit smoke
	l, err := newLayer(cfg, core.DTypeFloat32, quant.FormatNone, core.BackendCPUTiled)
	if err != nil {
		return err
	}
	...
	od, oh, ow := cfg.OutD(), cfg.OutH(), cfg.OutW()
	if len(post.Shape) != 5 || post.Shape[0] != 2 || post.Shape[1] != 5 || post.Shape[2] != od || post.Shape[3] != oh || post.Shape[4] != ow {
		return fmt.Errorf("post shape %v want [2,5,%d,%d,%d]", post.Shape, od, oh, ow)
	}
	fmt.Printf("([2,5,%d,%d,%d]) ", od, oh, ow)
	return nil
}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := cnn3.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
	master, ok := l.Proj.Weights.MasterF32()
	if !ok || len(master) == 0 {
...
func webGPUNoDevice() error {
	...
	_, _, err = cnn3.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func simdWebGPUAllQuants() error {
	...
	for _, f := range quant.AllFormats {
		...
		if f == quant.FormatAffinePacked {
			cfg := tinyCfg()
			if !suites.AffinePackable(cfg.Filters, cfg.PatchDim()) {
