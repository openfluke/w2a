...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	const batch, lr = 1, 1e-2
	cfg := tinyCfg() // shape-preserving
	...
	fmt.Printf("\n  CNN3 TRAIN — %s\n", title)
	fmt.Printf("  cell CNN3 C=%d %dx%dx%d→%dx%dx%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  CNN3 TRAIN — %s\n", title)
	fmt.Printf("  cell CNN3 C=%d %dx%dx%d→%dx%dx%d batch=%d  cells=%d\n\n",
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg cnn3.Config) (ns int64, status, note string) {
	...
	g, err := buildCNN3Cube(n, be, dt, format, cfg)
	if err != nil {
...
func buildCNN3Cube(n int, be core.Backend, dt core.DType, format quant.Format, cfg cnn3.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if err := cnn3.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg cnn3.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
	...
	st, nt := suites.StampBackendNote("cnn3", be == core.BackendSIMD, be == core.BackendWebGPU, "OK", "")
	return total.Nanoseconds() / int64(iters), st, nt
}

func trainBatch(batch int, cfg cnn3.Config) (x, y *core.Tensor[float32]) {
