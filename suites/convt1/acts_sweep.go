...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  ConvT1 activation numeric sweep — %d Tensor[T] kinds × %d backends\n", len(allActKinds), len(backends))
	fmt.Printf("  weights=FormatNone Float32  C=%d L=%d  SIMD=%v WebGPU=%v\n\n",
		tinyCfg().InChannels, tinyCfg().SeqLen, simd.Enabled(), webgpu.Available())
...
func smokeAct[T core.Numeric](be core.Backend) error {
	cfg := tinyCfg()
	init := make([]T, cfg.Filters*cfg.InChannels*cfg.Kernel)
	for i := range init {
		init[i] = core.FromFloat64[T](float64((i%5)-2) * 0.05)
	}
	l, err := convt1.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, init)
	if err != nil {
		return err
	}
	l.Exec.Backend = be
	l.Exec.MultiCore = false
	x := core.NewTensor[T](2, cfg.InChannels, cfg.SeqLen)
	for i := range x.Data {
