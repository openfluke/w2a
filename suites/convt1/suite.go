...
func tinyCfg() convt1.Config {
	return convt1.Config{
		InChannels: 4, Filters: 4, SeqLen: 8, Kernel: 3, Stride: 1,
		Activation: core.ActivationLinear,
	}
}

func defaultCfg() convt1.Config { return tinyCfg() }

func trainCfg() convt1.Config {
	return convt1.Config{
		InChannels: 4, Filters: 4, SeqLen: 8, Kernel: 1, Stride: 1,
		Activation: core.ActivationLinear,
	}
}

func affineOK(cfg convt1.Config) bool {
	return suites.AffinePackable(cfg.Filters, cfg.PatchDim())
}

func initWeights(cfg convt1.Config) []float32 {
...
func newLayer(cfg convt1.Config, dt core.DType, format quant.Format, be core.Backend) (*convt1.Layer, error) {
	...
	l, err := convt1.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initWeights(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg convt1.Config, batch int) *core.Tensor[float32] {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = convt1.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = convt1.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	_, post, err := convt1.Forward(l, makeInput(cfg, 1))
	if err != nil {
		return err
	}
	var sum float64
...
func shapeSmoke() error {
	...
	_, post, err := convt1.Forward(l, makeInput(cfg, 2))
	if err != nil {
		return err
	}
	want := cfg.OutLen()
...
func backwardFiniteDiff() error {
	...
	_, dW, err := convt1.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
...
func webGPUNoDevice() error {
	...
	_, _, err = convt1.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func cpuTiledFormatNoneAll() error {
	...
	for _, dt := range core.AllDTypes {
		if !convt1.PermutationOK(dt, quant.FormatNone, core.BackendCPUTiled) {
			continue
		}
...
func simdFormatNoneAll() error {
	...
	var fails []string
	for _, dt := range core.AllDTypes {
		if !convt1.PermutationOK(dt, quant.FormatNone, core.BackendSIMD) {
			continue
		}
...
func simdWebGPUAllQuants() error {
	...
	for _, f := range quant.AllFormats {
		...
		for _, be := range backends {
			...
			if !convt1.PermutationOK(core.DTypeFloat32, f, be) {
				continue
			}
