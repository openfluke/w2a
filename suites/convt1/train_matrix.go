...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  ConvT1 TRAIN — %s\n", title)
	fmt.Printf("  cell ConvT1 C=%d L=%d→%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
		cfg.InChannels, cfg.SeqLen, cfg.OutLen(), batch, total, simd.Enabled(), webgpu.Available())
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  ConvT1 TRAIN — %s\n", title)
	fmt.Printf("  cell ConvT1 C=%d L=%d→%d batch=%d  cells=%d\n\n",
		cfg.InChannels, cfg.SeqLen, cfg.OutLen(), batch, total)
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg convt1.Config) (ns int64, status, note string) {
...
func buildCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg convt1.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if err := convt1.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg convt1.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
...
func trainBatch(batch int, cfg convt1.Config) (x, y *core.Tensor[float32]) {
	x = core.NewTensor[float32](batch, cfg.InChannels, cfg.SeqLen)
	y = core.NewTensor[float32](batch, cfg.Filters, cfg.OutLen())
	for i := range x.Data {
