...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  ConvT2 activation numeric sweep — %d Tensor[T] kinds × %d backends\n", len(allActKinds), len(backends))
	fmt.Printf("  weights=FormatNone Float32  C=%d HxW=%dx%d  SIMD=%v WebGPU=%v\n\n",
...
func smokeAct[T core.Numeric](be core.Backend) error {
	...
	l, err := convt2.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, init)
	if err != nil {
		return err
	}
	l.Exec.Backend = be
