...
func tinyCfg() convt2.Config {
	return convt2.Config{
		InChannels: 4, Filters: 4, Height: 4, Width: 4, Kernel: 3, Stride: 1,
		Activation: core.ActivationLinear,
	}
}

func defaultCfg() convt2.Config { return tinyCfg() }

func trainCfg() convt2.Config {
	return convt2.Config{
		InChannels: 4, Filters: 4, Height: 4, Width: 4, Kernel: 1, Stride: 1,
		Activation: core.ActivationLinear,
	}
}

func affineOK(cfg convt2.Config) bool {
	return suites.AffinePackable(cfg.Filters, cfg.PatchDim())
}

func initWeights(cfg convt2.Config) []float32 {
...
func newLayer(cfg convt2.Config, dt core.DType, format quant.Format, be core.Backend) (*convt2.Layer, error) {
	...
	l, err := convt2.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initWeights(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg convt2.Config, batch int) *core.Tensor[float32] {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = convt2.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = convt2.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	_, post, err := convt2.Forward(l, makeInput(cfg, 1))
	if err != nil {
		return err
	}
	var sum float64
...
func shapeSmoke() error {
	...
	_, post, err := convt2.Forward(l, makeInput(cfg, 2))
	if err != nil {
		return err
	}
	if len(post.Shape) != 4 || post.Shape[0] != 2 || post.Shape[1] != 5 || post.Shape[2] != cfg.OutH() || post.Shape[3] != cfg.OutW() {
		return fmt.Errorf("post shape %v want [2,5,%d,%d]", post.Shape, cfg.OutH(), cfg.OutW())
	}
	fmt.Printf("([2,5,%d,%d]) ", cfg.OutH(), cfg.OutW())
	return nil
}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := convt2.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
...
func webGPUNoDevice() error {
	...
	_, _, err = convt2.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func cpuTiledFormatNoneAll() error {
	...
	for _, dt := range core.AllDTypes {
		if !convt2.PermutationOK(dt, quant.FormatNone, core.BackendCPUTiled) {
			continue
		}
...
func simdFormatNoneAll() error {
	...
	var fails []string
	for _, dt := range core.AllDTypes {
		if !convt2.PermutationOK(dt, quant.FormatNone, core.BackendSIMD) {
			continue
		}
...
func simdWebGPUAllQuants() error {
	...
	for _, f := range quant.AllFormats {
		...
		for _, be := range backends {
			...
			if !convt2.PermutationOK(core.DTypeFloat32, f, be) {
				continue
			}
