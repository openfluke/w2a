...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  ConvT3 activation numeric sweep — %d Tensor[T] kinds × %d backends\n", len(allActKinds), len(backends))
	fmt.Printf("  weights=FormatNone Float32  C=%d DxHxW=%dx%dx%d  SIMD=%v WebGPU=%v\n\n",
...
func smokeAct[T core.Numeric](be core.Backend) error {
	cfg := tinyCfg()
	init := make([]T, cfg.Filters*cfg.PatchDim())
	for i := range init {
		init[i] = core.FromFloat64[T](float64((i%5)-2) * 0.05)
	}
	l, err := convt3.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, init)
	if err != nil {
		return err
	}
	l.Exec.Backend = be
