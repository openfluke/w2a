...
func TimedMatrix() error {
	...
	fmt.Printf("\n  ConvT3 timed matrix — FormatNone × %d dtypes × %d backends\n", len(core.AllDTypes), len(backends))
...
func TimedQuantMatrix() error {
	...
	cfg := defaultCfg()
	backends := []core.Backend{core.BackendCPUTiled, core.BackendSIMD, core.BackendWebGPU}
	fmt.Printf("\n  ConvT3 timed quant matrix — %d formats × %d backends (Float32)\n", len(quant.AllFormats), len(backends))
	fmt.Printf("  shape batch=%d C=%d %dx%dx%d→%dx%dx%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.InChannels, cfg.Depth, cfg.Height, cfg.Width, cfg.OutD(), cfg.OutH(), cfg.OutW(), simd.Enabled(), webgpu.Available())
...
func timeCell(dt core.DType, format quant.Format, be core.Backend, cfg convt3.Config, batch, warm, iters int) (fwdNs, bwdNs int64, status, note string) {
	...
	for i := 0; i < warm; i++ {
		pre, _, err = convt3.Forward(l, x)
		if err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
		if _, _, err = convt3.Backward(l, gOut, x, pre); err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
	}
	if iters < 1 {
