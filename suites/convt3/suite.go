...
func tinyCfg() convt3.Config {
	return convt3.Config{
		InChannels: 2, Filters: 2, Depth: 2, Height: 2, Width: 2, Kernel: 2, Stride: 1,
		Activation: core.ActivationLinear,
	}
}

func defaultCfg() convt3.Config { return tinyCfg() }

func trainCfg() convt3.Config {
	return convt3.Config{
		InChannels: 2, Filters: 2, Depth: 2, Height: 2, Width: 2, Kernel: 1, Stride: 1,
		Activation: core.ActivationLinear,
	}
}

func affineOK(cfg convt3.Config) bool {
	return suites.AffinePackable(cfg.Filters, cfg.PatchDim())
}

func initWeights(cfg convt3.Config) []float32 {
	n := cfg.Filters * cfg.PatchDim()
	w := make([]float32, n)
	for i := range w {
...
func newLayer(cfg convt3.Config, dt core.DType, format quant.Format, be core.Backend) (*convt3.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	if !convt3.PermutationOK(dt, format, be) {
		return nil, fmt.Errorf("unsupported: %s/%s/%s", dt, format, be)
	}
	l, err := convt3.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initWeights(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg convt3.Config, batch int) *core.Tensor[float32] {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = convt3.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = convt3.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	_, post, err := convt3.Forward(l, makeInput(cfg, 1))
	if err != nil {
		return err
	}
	var sum float64
...
func shapeSmoke() error {
	...
	_, post, err := convt3.Forward(l, makeInput(cfg, 2))
	if err != nil {
		return err
	}
	if len(post.Shape) != 5 || post.Shape[0] != 2 || post.Shape[1] != 3 || post.Shape[2] != cfg.OutD() || post.Shape[3] != cfg.OutH() || post.Shape[4] != cfg.OutW() {
		return fmt.Errorf("post shape %v want [2,3,%d,%d,%d]", post.Shape, cfg.OutD(), cfg.OutH(), cfg.OutW())
	}
	fmt.Printf("([2,3,%d,%d,%d]) ", cfg.OutD(), cfg.OutH(), cfg.OutW())
	return nil
}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := convt3.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
...
func webGPUNoDevice() error {
	...
	_, _, err = convt3.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func cpuTiledFormatNoneAll() error {
	...
	for _, dt := range core.AllDTypes {
		if !convt3.PermutationOK(dt, quant.FormatNone, core.BackendCPUTiled) {
			continue
		}
...
func simdFormatNoneAll() error {
	...
	var fails []string
	for _, dt := range core.AllDTypes {
		if !convt3.PermutationOK(dt, quant.FormatNone, core.BackendSIMD) {
			continue
		}
...
func simdWebGPUAllQuants() error {
	...
	for _, f := range quant.AllFormats {
		...
		for _, be := range backends {
			...
			if !convt3.PermutationOK(core.DTypeFloat32, f, be) {
				continue
			}
