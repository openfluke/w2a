...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  ConvT3 TRAIN — %s\n", title)
	fmt.Printf("  cell ConvT3 C=%d %dx%dx%d→%dx%dx%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
		cfg.InChannels, cfg.Depth, cfg.Height, cfg.Width, cfg.OutD(), cfg.OutH(), cfg.OutW(), batch, total, simd.Enabled(), webgpu.Available())
	...
	for _, n := range sizes {
		...
		for _, dt := range core.AllDTypes {
			for _, be := range backends {
				ns, status, note := timeTrainCube(n, be, batch, warm, iters, lr, dt, quant.FormatNone, cfg)
	...
	if len(cpuFails) > 0 {
		return fmt.Errorf("train FormatNone grids: %d failures: %s", len(cpuFails), strings.Join(cpuFails[:min(8, len(cpuFails))], " | "))
	}
	return nil
}
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  ConvT3 TRAIN — %s\n", title)
	fmt.Printf("  cell ConvT3 C=%d %dx%dx%d→%dx%dx%d batch=%d  cells=%d\n\n",
		cfg.InChannels, cfg.Depth, cfg.Height, cfg.Width, cfg.OutD(), cfg.OutH(), cfg.OutW(), batch, total)
	...
	for _, n := range sizes {
		...
		for _, f := range quant.AllFormats {
			for _, be := range backends {
				ns, status, note := timeTrainCube(n, be, batch, warm, iters, lr, core.DTypeFloat32, f, cfg)
				grid := fmt.Sprintf("%dx%dx%d", n, n, n)
	...
	fmt.Printf("\n  summary: %d OK, %d GAP, %d FAIL (of %d cells)\n", okN, gapN, len(cpuFails), total)
	if len(cpuFails) > 0 {
		return fmt.Errorf("train quant grids: %d failures: %s", len(cpuFails), strings.Join(cpuFails[:min(8, len(cpuFails))], " | "))
	}
	return nil
}
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg convt3.Config) (ns int64, status, note string) {
	...
	if !convt3.PermutationOK(dt, format, be) {
		return 0, "GAP", "unsupported permutation"
	}
	g, err := buildCube(n, be, dt, format, cfg)
	if err != nil {
		return 0, failOrGap(be), err.Error()
	}
	return benchTrainStep(g, batch, cfg, warm, iters, lr, be)
}

func buildCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg convt3.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if w, ok := l.Proj.Weights.MasterF32(); ok && len(w) > 0 {
					w[0] += float32(z+y+x) * 0.001
				}
				if err := convt3.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg convt3.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
	x, y := trainBatch(batch, cfg)
	for i := 0; i < warm; i++ {
...
func trainBatch(batch int, cfg convt3.Config) (x, y *core.Tensor[float32]) {
	x = core.NewTensor[float32](batch, cfg.InChannels, cfg.Depth, cfg.Height, cfg.Width)
	y = core.NewTensor[float32](batch, cfg.Filters, cfg.OutD(), cfg.OutH(), cfg.OutW())
	for i := range x.Data {
