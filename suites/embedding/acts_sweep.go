...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  Embedding activation numeric sweep — %d Tensor[T] kinds × %d backends\n",
		len(allActKinds), len(backends))
	fmt.Printf("  weights=FormatNone Float32  V=%d E=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		tinyCfg().VocabSize, tinyCfg().EmbeddingDim, tinyCfg().SeqLen, simd.Enabled(), webgpu.Available())
...
func smokeAct[T core.Numeric](be core.Backend) error {
	...
	for i := range packed {
		packed[i] = core.FromFloat64[T](float64((i%5)-2) * 0.05)
	}
	l, err := embedding.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, packed)
	if err != nil {
		return err
	}
	l.Exec.Backend = be
	l.Exec.MultiCore = false
	x := core.NewTensor[T](2, cfg.SeqLen)
	for i := range x.Data {
		x.Data[i] = core.FromFloat64[T](float64(i % cfg.VocabSize))
	}
