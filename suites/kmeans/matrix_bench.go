...
func TimedMatrix() error {
	...
	fmt.Printf("\n  KMeans timed matrix — FormatNone × %d dtypes × %d backends\n", len(core.AllDTypes), len(backends))
	fmt.Printf("  shape batch=%d K=%d D=%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.NumClusters, cfg.FeatureDim, simd.Enabled(), webgpu.Available())
...
func TimedQuantMatrix() error {
	...
	cfg := defaultCfg()
	backends := []core.Backend{core.BackendCPUTiled, core.BackendSIMD, core.BackendWebGPU}
	fmt.Printf("\n  KMeans timed quant matrix — %d formats × %d backends (Float32)\n", len(quant.AllFormats), len(backends))
	fmt.Printf("  shape batch=%d K=%d D=%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.NumClusters, cfg.FeatureDim, simd.Enabled(), webgpu.Available())
...
func timeCell(dt core.DType, format quant.Format, be core.Backend, cfg kmeans.Config, batch, warm, iters int) (fwdNs, bwdNs int64, status, note string) {
	...
	gOut := core.NewTensor[float32](batch, cfg.OutDim())
	for i := range gOut.Data {
		gOut.Data[i] = 1
	}
	var pre *core.Tensor[float32]
	for i := 0; i < warm; i++ {
		pre, _, err = kmeans.Forward(l, x)
		if err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
		if _, _, err = kmeans.Backward(l, gOut, x, pre); err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
	}
	if iters < 1 {
