...
// matrix/smoke: probs mode K=8 D=64 (AffinePackable)
func tinyCfg() kmeans.Config {
	return kmeans.Config{
		NumClusters: 8, FeatureDim: 64, Temperature: 1,
		OutputMode: kmeans.OutputProbabilities, Activation: core.ActivationLinear,
	}
}

func defaultCfg() kmeans.Config { return tinyCfg() }

// train cubes: shape-preserving FeatureDim→FeatureDim via OutputFeatures K=64
func trainCfg() kmeans.Config {
	return kmeans.Config{
		NumClusters: 64, FeatureDim: 64, Temperature: 1,
		OutputMode: kmeans.OutputFeatures, Activation: core.ActivationLinear,
	}
}

func affineOK(cfg kmeans.Config) bool {
	return suites.AffinePackable(cfg.NumClusters, cfg.FeatureDim)
}

func initWeights(cfg kmeans.Config) []float32 {
	n := cfg.NumClusters * cfg.FeatureDim
	w := make([]float32, n)
	for i := range w {
...
func newLayer(cfg kmeans.Config, dt core.DType, format quant.Format, be core.Backend) (*kmeans.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	if !kmeans.PermutationOK(dt, format, be) {
		return nil, fmt.Errorf("unsupported: %s/%s/%s", dt, format, be)
	}
	l, err := kmeans.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initWeights(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg kmeans.Config, batch int) *core.Tensor[float32] {
	x := core.NewTensor[float32](batch, cfg.FeatureDim)
	for i := range x.Data {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = kmeans.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = kmeans.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	_, post, err := kmeans.Forward(l, makeInput(cfg, 1))
	if err != nil {
		return err
	}
	var sum float64
	...
	fmt.Printf("(K=%d D=%d) ", cfg.NumClusters, cfg.FeatureDim)
	return nil
}
...
func shapeSmoke() error {
	...
	_, post, err := kmeans.Forward(l, makeInput(cfg, 2))
	if err != nil {
		return err
	}
	if len(post.Shape) != 2 || post.Shape[0] != 2 || post.Shape[1] != cfg.NumClusters {
		return fmt.Errorf("post shape %v want [2,%d]", post.Shape, cfg.NumClusters)
	}
	fmt.Printf("([2,%d]) ", cfg.NumClusters)
	return nil
}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := kmeans.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
	master, ok := l.Centers.Weights.MasterF32()
	if !ok || len(master) == 0 {
...
func webGPUNoDevice() error {
	...
	_, _, err = kmeans.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func cpuTiledFormatNoneAll() error {
	...
	for _, dt := range core.AllDTypes {
		if !kmeans.PermutationOK(dt, quant.FormatNone, core.BackendCPUTiled) {
			continue
		}
...
func simdFormatNoneAll() error {
	...
	var fails []string
	for _, dt := range core.AllDTypes {
		if !kmeans.PermutationOK(dt, quant.FormatNone, core.BackendSIMD) {
			continue
		}
...
func simdWebGPUAllQuants() error {
	...
	for _, f := range quant.AllFormats {
		...
		for _, be := range backends {
			...
			if !kmeans.PermutationOK(core.DTypeFloat32, f, be) {
				continue
			}
