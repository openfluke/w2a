...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  KMeans TRAIN — %s\n", title)
	fmt.Printf("  cell KMeans D=%d K=%d mode=%s batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
		cfg.FeatureDim, cfg.NumClusters, cfg.OutputMode, batch, total, simd.Enabled(), webgpu.Available())
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  KMeans TRAIN — %s\n", title)
	fmt.Printf("  cell KMeans D=%d K=%d mode=%s batch=%d  cells=%d\n\n",
		cfg.FeatureDim, cfg.NumClusters, cfg.OutputMode, batch, total)
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg kmeans.Config) (ns int64, status, note string) {
	...
	g, err := buildKMeansCube(n, be, dt, format, cfg)
	if err != nil {
...
func buildKMeansCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg kmeans.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if w, ok := l.Centers.Weights.MasterF32(); ok && len(w) > 0 {
					w[0] += float32(z+y+x) * 0.001
				}
				if err := kmeans.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg kmeans.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
...
func trainBatch(batch int, cfg kmeans.Config) (x, y *core.Tensor[float32]) {
	x = core.NewTensor[float32](batch, cfg.FeatureDim)
	y = core.NewTensor[float32](batch, cfg.OutDim())
	for i := range x.Data {
