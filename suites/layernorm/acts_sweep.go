...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  LayerNorm activation numeric sweep — %d Tensor[T] kinds × %d backends\n",
...
func smokeAct[T core.Numeric](be core.Backend) error {
	cfg := tinyCfg()
	l, err := layernorm.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, onesInitT[T](cfg.Dim), zerosInitT[T](cfg.Dim))
	if err != nil {
		return err
	}
	l.Exec.Backend = be
...
func onesInitT[T core.Numeric](dim int) []T {
	w := make([]T, dim)
	for i := range w {
		w[i] = core.FromFloat64[T](1)
	}
	return w
}

func zerosInitT[T core.Numeric](dim int) []T {
	return make([]T, dim)
}
