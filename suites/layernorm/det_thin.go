...
func repeatForwardDet() error {
	...
	if err := suites.RequireDet("layernorm repeat", max, suites.DetTolFwd); err != nil {
		return err
	}
...
func scmcFwdBwdDet() error {
	...
	run := func(multi bool) (post, dW []float32, err error) {
		...
		pre, p, err := layernorm.Forward(l, x)
		if err != nil {
			return nil, nil, err
		}
	...
	if err := suites.RequireDet("layernorm fwd SC↔MC", dP, suites.DetTolFwd); err != nil {
		return err
	}
	if err := suites.RequireDet("layernorm bwd SC↔MC", dW, suites.DetTolBwd); err != nil {
		return err
	}
