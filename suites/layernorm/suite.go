...
func Cases() []Case {
	return []Case{
		{Name: "Forward smoke (γ≈1 β≈0)", Run: forwardSmoke},
		{Name: "Seq layout [batch,seq,dim] smoke", Run: seqLayoutSmoke},
		{Name: "Backward finite-diff dγ/dβ spot-check", Run: backwardFiniteDiff},
		...
		{Name: "§12 SIMD full scale CPU↔SIMD parity", Run: lnSimdScaleParity},
		{Name: "§12 WebGPU LayerNorm bwd CPU↔GPU parity", Run: layerNormWebGPUBwdParity},
...
func RunAll() error {
	...
	if len(fails) > 0 {
		return fmt.Errorf("layernorm: %d failed: %s", len(fails), strings.Join(fails, ", "))
	}
	return nil
}
...
func RunOne(n int) error {
	cs := Cases()
	if n < 1 || n > len(cs) {
		return fmt.Errorf("layernorm: case %d out of range 1..%d", n, len(cs))
	}
	suites.BeginCase()
	fmt.Printf("  [%d] %s … ", n, cs[n-1].Name)
	if err := cs[n-1].Run(); err != nil {
		suites.EndCase("layernorm", cs[n-1].Name, "FAIL", err.Error())
		fmt.Printf("FAIL\n      %v\n", err)
		return err
	}
	suites.EndCase("layernorm", cs[n-1].Name, "PASS", "")
	fmt.Println("PASS")
	return nil
}
...
func zerosInit(dim int) []float32 {
	return make([]float32, dim)
}

func newLayer(cfg layernorm.Config, dt core.DType, format quant.Format, be core.Backend) (*layernorm.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	l, err := layernorm.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, onesInit(cfg.Dim), zerosInit(cfg.Dim))
	if err != nil {
		return nil, err
	}
...
func makeInput(batch, dim int) *core.Tensor[float32] {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = layernorm.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = layernorm.Forward(l, makeInput(2, tinyCfg().Dim))
	return err
}
...
func forwardSmoke() error {
	...
	fmt.Printf("(dim=%d γ≈1) ", tinyCfg().Dim)
	return nil
}
...
func seqLayoutSmoke() error {
	...
	x := core.NewTensor[float32](2, 3, cfg.Dim)
	for i := range x.Data {
	...
	if len(post.Shape) != 3 || post.Shape[0] != 2 || post.Shape[1] != 3 || post.Shape[2] != cfg.Dim {
		return fmt.Errorf("post shape %v", post.Shape)
	}
	fmt.Printf("([2,3,%d]) ", cfg.Dim)
	return nil
}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := layernorm.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
	master, ok := l.Gamma.MasterF32()
	if !ok || len(master) == 0 {
...
func gradVerifyBackends() error {
	...
	if max > 1e-2 {
		return fmt.Errorf("CPU↔SIMD dγ/dβ max err %v", max)
	}
...
func webGPUNoDevice() error {
	...
	_, _, err = layernorm.Forward(l, makeInput(1, tinyCfg().Dim))
	if err == nil {
...
func fullMatrixGaps() error {
	perms := layernorm.AllPermutations()
	var failN int
	var samples []string
	for _, p := range perms {
		var err error
