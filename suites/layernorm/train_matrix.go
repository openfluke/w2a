...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  LayerNorm TRAIN — %s\n", title)
	fmt.Printf("  cell LayerNorm dim=%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  LayerNorm TRAIN — %s\n", title)
	fmt.Printf("  cell LayerNorm dim=%d batch=%d  cells=%d\n\n",
		cfg.Dim, batch, total)
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg layernorm.Config) (ns int64, status, note string) {
	...
	g, err := buildLayerNormCube(n, be, dt, format, cfg)
	if err != nil {
		return 0, failOrGap(be), err.Error()
	}
	return benchTrainStep(g, batch, cfg.Dim, warm, iters, lr, be)
}

func buildLayerNormCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg layernorm.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if w, ok := l.Gamma.MasterF32(); ok && len(w) > 0 {
					w[0] += float32(z+y+x) * 0.001
				}
				if w, ok := l.Beta.MasterF32(); ok && len(w) > 0 {
					w[0] += float32(z+y+x) * 0.0005
				}
				if err := layernorm.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
