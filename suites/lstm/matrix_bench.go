...
func TimedMatrix() error {
	...
	fmt.Printf("\n  LSTM timed matrix — FormatNone × %d dtypes × %d backends\n", len(core.AllDTypes), len(backends))
...
func TimedQuantMatrix() error {
	...
	fmt.Printf("\n  LSTM timed quant matrix — %d formats × %d backends (Float32)\n", len(quant.AllFormats), len(backends))
	fmt.Printf("  shape batch=%d in=%d hid=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.InputSize, cfg.HiddenSize, cfg.SeqLen, simd.Enabled(), webgpu.Available())
...
func timeCell(dt core.DType, format quant.Format, be core.Backend, cfg lstm.Config, batch, warm, iters int) (fwdNs, bwdNs int64, status, note string) {
	...
	for i := 0; i < warm; i++ {
		pre, _, err = lstm.Forward(l, x)
		if err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
		if _, _, err = lstm.Backward(l, gOut, x, pre); err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
	}
	if iters < 1 {
	...
	st, nt := suites.StampBackendNote("lstm", be == core.BackendSIMD, be == core.BackendWebGPU, "OK", "")
