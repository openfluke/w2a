...
func Cases() []Case {
	return []Case{
		{Name: "Forward smoke (LSTM gates)", Run: forwardSmoke},
...
func RunAll() error {
	...
	if len(fails) > 0 {
		return fmt.Errorf("lstm: %d failed: %s", len(fails), strings.Join(fails, ", "))
	}
	return nil
}
...
func RunOne(n int) error {
	cs := Cases()
	if n < 1 || n > len(cs) {
		return fmt.Errorf("lstm: case %d out of range 1..%d", n, len(cs))
	}
	suites.BeginCase()
	fmt.Printf("  [%d] %s … ", n, cs[n-1].Name)
	if err := cs[n-1].Run(); err != nil {
		suites.EndCase("lstm", cs[n-1].Name, "FAIL", err.Error())
		fmt.Printf("FAIL\n      %v\n", err)
		return err
	}
	suites.EndCase("lstm", cs[n-1].Name, "PASS", "")
	fmt.Println("PASS")
	return nil
}
...
func newLayer(cfg lstm.Config, dt core.DType, format quant.Format, be core.Backend) (*lstm.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	l, err := lstm.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initPacked(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg lstm.Config, batch int) *core.Tensor[float32] {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = lstm.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = lstm.Forward(l, makeInput(cfg, 2))
	return err
}
...
func backwardFiniteDiff() error {
	...
	master, ok := l.I.IH.Weights.MasterF32()
	if !ok || len(master) == 0 {
...
func webGPUNoDevice() error {
	...
	_, _, err = lstm.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
