...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  Mamba activation numeric sweep — %d Tensor[T] kinds × %d backends\n",
		len(allActKinds), len(backends))
	fmt.Printf("  weights=FormatNone Float32  D=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		tinyCfg().DModel, tinyCfg().SeqLen, simd.Enabled(), webgpu.Available())
...
func smokeAct[T core.Numeric](be core.Backend) error {
	cfg := tinyCfg()
	inner := cfg.InnerDim()
	inInit := make([]T, 2*inner*cfg.DModel)
	outInit := make([]T, cfg.DModel*inner)
	for i := range inInit {
		inInit[i] = core.FromFloat64[T](float64((i%5)-2) * 0.02)
	}
	for i := range outInit {
		outInit[i] = core.FromFloat64[T](float64((i%5)-2) * 0.02)
	}
	...
	x := core.NewTensor[T](2, cfg.SeqLen, cfg.DModel)
	for i := range x.Data {
