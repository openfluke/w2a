...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Mamba TRAIN — %s\n", title)
	fmt.Printf("  cell Mamba D=%d T=%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
		cfg.DModel, cfg.SeqLen, batch, total, simd.Enabled(), webgpu.Available())
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Mamba TRAIN — %s\n", title)
	fmt.Printf("  cell Mamba D=%d T=%d batch=%d  cells=%d\n\n",
		cfg.DModel, cfg.SeqLen, batch, total)
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg mamba.Config) (ns int64, status, note string) {
	...
	g, err := buildMambaCube(n, be, dt, format, cfg)
	if err != nil {
...
func buildMambaCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg mamba.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if w, ok := l.InProj.Weights.MasterF32(); ok && len(w) > 0 {
					w[0] += float32(z+y+x) * 0.001
				}
				if err := mamba.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg mamba.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
...
func trainBatch(batch int, cfg mamba.Config) (x, y *core.Tensor[float32]) {
	x = core.NewTensor[float32](batch, cfg.SeqLen, cfg.DModel)
	y = core.NewTensor[float32](batch, cfg.SeqLen, cfg.DModel)
	for i := range x.Data {
