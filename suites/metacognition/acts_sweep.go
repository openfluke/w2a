...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  Metacognition activation numeric sweep — %d Tensor[T] kinds × %d backends\n", len(allActKinds), len(backends))
...
func smokeAct[T core.Numeric](be core.Backend) error {
	cfg := tinyCfg()
	init := make([]T, cfg.Dim*cfg.Dim)
	for i := range init {
		init[i] = core.FromFloat64[T](float64((i%5)-2) * 0.02)
	}
	l, err := metacognition.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, init)
	if err != nil {
		return err
	}
	l.Exec.Backend = be
