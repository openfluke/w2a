...
func tinyCfg() metacognition.Config {
	return metacognition.Config{Dim: 64, Rules: metacognition.DefaultStabilityRules()}
}

func defaultCfg() metacognition.Config { return tinyCfg() }

func affineOK(cfg metacognition.Config) bool {
	return suites.AffinePackable(cfg.Dim, cfg.Dim)
}

func initWeights(cfg metacognition.Config) []float32 {
	n := cfg.Dim * cfg.Dim
	w := make([]float32, n)
	for i := range w {
...
func newLayer(cfg metacognition.Config, dt core.DType, format quant.Format, be core.Backend) (*metacognition.Layer, error) {
	...
	l, err := metacognition.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initWeights(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg metacognition.Config, batch int) *core.Tensor[float32] {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = metacognition.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = metacognition.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	_, post, err := metacognition.Forward(l, makeInput(cfg, 1))
	if err != nil {
		return err
	}
	var sum float64
...
func shapeSmoke() error {
	...
	_, post, err := metacognition.Forward(l, makeInput(cfg, 2))
	if err != nil {
		return err
	}
	if len(post.Shape) != 2 || post.Shape[0] != 2 || post.Shape[1] != cfg.Dim {
		return fmt.Errorf("post shape %v want [2,%d]", post.Shape, cfg.Dim)
	}
	fmt.Printf("([2,%d]) ", cfg.Dim)
	return nil
}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := metacognition.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
	master, ok := l.Observed.Weights.MasterF32()
	if !ok || len(master) == 0 {
	...
	lossAt := func() (float64, error) {
		_, p, err := metacognition.Forward(l, x)
		if err != nil {
			return 0, err
		}
		var loss float64
...
func webGPUNoDevice() error {
	...
	_, _, err = metacognition.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func cpuTiledFormatNoneAll() error {
	...
	for _, dt := range core.AllDTypes {
		if !metacognition.PermutationOK(dt, quant.FormatNone, core.BackendCPUTiled) {
			continue
		}
...
func simdFormatNoneAll() error {
	...
	var fails []string
	for _, dt := range core.AllDTypes {
		if !metacognition.PermutationOK(dt, quant.FormatNone, core.BackendSIMD) {
			continue
		}
...
func simdWebGPUAllQuants() error {
	...
	for _, f := range quant.AllFormats {
		...
		for _, be := range backends {
			...
			if !metacognition.PermutationOK(core.DTypeFloat32, f, be) {
				continue
			}
