...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Metacognition TRAIN — %s\n", title)
	fmt.Printf("  cell Metacognition Dim=%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
		cfg.Dim, batch, total, simd.Enabled(), webgpu.Available())
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Metacognition TRAIN — %s\n", title)
	fmt.Printf("  cell Metacognition Dim=%d batch=%d  cells=%d\n\n", cfg.Dim, batch, total)
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg metacognition.Config) (ns int64, status, note string) {
	...
	if !metacognition.PermutationOK(dt, format, be) {
		return 0, "GAP", "unsupported permutation"
	}
	g, err := buildMetaCube(n, be, dt, format, cfg)
	if err != nil {
...
func buildMetaCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg metacognition.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if w, ok := l.Observed.Weights.MasterF32(); ok && len(w) > 0 {
					w[0] += float32(z+y+x) * 0.001
				}
				if err := metacognition.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg metacognition.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
...
func trainBatch(batch int, cfg metacognition.Config) (x, y *core.Tensor[float32]) {
	x = core.NewTensor[float32](batch, cfg.Dim)
	y = core.NewTensor[float32](batch, cfg.Dim)
	for i := range x.Data {
