...
func TimedMatrix() error {
	...
	fmt.Printf("\n  Parallel timed matrix — FormatNone × %d dtypes × %d backends\n", len(core.AllDTypes), len(backends))
	fmt.Printf("  shape batch=%d Dim=%d Out=%d Br=%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.Dim, cfg.OutFeat, cfg.Branches, simd.Enabled(), webgpu.Available())
...
func TimedQuantMatrix() error {
	...
	cfg := defaultCfg()
	backends := []core.Backend{core.BackendCPUTiled, core.BackendSIMD, core.BackendWebGPU}
	fmt.Printf("\n  Parallel timed quant matrix — %d formats × %d backends (Float32)\n", len(quant.AllFormats), len(backends))
	fmt.Printf("  shape batch=%d Dim=%d Out=%d Br=%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.Dim, cfg.OutFeat, cfg.Branches, simd.Enabled(), webgpu.Available())
...
func timeCell(dt core.DType, format quant.Format, be core.Backend, cfg parallel.Config, batch, warm, iters int) (fwdNs, bwdNs int64, status, note string) {
	...
	for i := 0; i < warm; i++ {
		pre, _, err = parallel.Forward(l, x)
		if err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
		if _, _, err = parallel.Backward(l, gOut, x, pre); err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
	}
	if iters < 1 {
