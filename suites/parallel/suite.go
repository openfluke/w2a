...
func Cases() []Case {
	return []Case{
		...
		{Name: "NESTED — Sequential mixed Dense/RMSNorm/SwiGLU", Run: NestedMixedSequentialSmoke},
		{Name: "NESTED — Residual mixed F + Parallel ResidualGraft", Run: NestedMixedResidualSmoke},
		...
		{Name: "CAMERAL CREDIT — Split/FastProxy/Sparse/Alt × Bi/hemi/combine/poly", Run: CameralCreditModes},
		{Name: "CAMERAL CREDIT GRIDS — Split/FastProxy/Sparse/Alt/Mesh × 1³/2³/3³", Run: CameralCreditTrainGrids},
		{Name: "TEST49 — all train modes × 1³/2³/3³ × all layer kinds (origin smoke)", Run: Test49TrainGrids},
		...
		{Name: "CAMERAL PERM — FormatNone × dtypes × backends × test41+credit+mesh × arches", Run: CameralPermMatrixFormatNone},
		{Name: "CAMERAL PERM — quants × backends × test41+credit+mesh × arches (Float32)", Run: CameralPermMatrixQuant},
	}
}
...
func tinyCfg() parallel.Config {
	return parallel.Config{Dim: 64, OutFeat: 64, Branches: 2, Combine: parallel.CombineAdd}
}

func defaultCfg() parallel.Config { return tinyCfg() }

func affineOK(cfg parallel.Config) bool {
	return suites.AffinePackable(cfg.OutFeat, cfg.Dim)
}

func initPacked(cfg parallel.Config) []float32 {
	n := cfg.Branches * cfg.OutFeat * cfg.Dim
	w := make([]float32, n)
	for i := range w {
...
func newLayer(cfg parallel.Config, dt core.DType, format quant.Format, be core.Backend) (*parallel.Layer, error) {
	...
	l, err := parallel.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initPacked(cfg), nil)
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg parallel.Config, batch int) *core.Tensor[float32] {
	x := core.NewTensor[float32](batch, cfg.Dim)
	for i := range x.Data {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = parallel.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	_, post, err := parallel.Forward(l, makeInput(cfg, 1))
	if err != nil {
		return err
	}
	var sum float64
	...
	fmt.Printf("(Dim=%d Out=%d Br=%d) ", cfg.Dim, cfg.OutFeat, cfg.Branches)
	return nil
}
...
func shapeSmoke() error {
	...
	_, post, err := parallel.Forward(l, makeInput(cfg, 2))
	if err != nil {
		return err
	}
	want := cfg.OutDim()
	if len(post.Shape) != 2 || post.Shape[0] != 2 || post.Shape[1] != want {
		return fmt.Errorf("post shape %v want [2,%d]", post.Shape, want)
	}
	fmt.Printf("([2,%d]) ", want)
	return nil
}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := parallel.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
...
func webGPUNoDevice() error {
	...
	_, _, err = parallel.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func cpuTiledFormatNoneAll() error {
	...
	for _, dt := range core.AllDTypes {
		if !parallel.PermutationOK(dt, quant.FormatNone, core.BackendCPUTiled) {
			continue
		}
...
func simdFormatNoneAll() error {
	...
	var fails []string
	for _, dt := range core.AllDTypes {
		if !parallel.PermutationOK(dt, quant.FormatNone, core.BackendSIMD) {
			continue
		}
...
func simdWebGPUAllQuants() error {
	...
	for _, f := range quant.AllFormats {
		...
		for _, be := range backends {
			...
			if !parallel.PermutationOK(core.DTypeFloat32, f, be) {
				continue
			}
