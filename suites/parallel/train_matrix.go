...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Parallel TRAIN — %s\n", title)
	fmt.Printf("  cell Parallel Dim=%d Out=%d Br=%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
		cfg.Dim, cfg.OutFeat, cfg.Branches, batch, total, simd.Enabled(), webgpu.Available())
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Parallel TRAIN — %s\n", title)
	fmt.Printf("  cell Parallel Dim=%d Out=%d Br=%d batch=%d  cells=%d\n\n",
		cfg.Dim, cfg.OutFeat, cfg.Branches, batch, total)
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg parallel.Config) (ns int64, status, note string) {
	...
	g, err := buildParallelCube(n, be, dt, format, cfg)
	if err != nil {
...
func buildParallelCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg parallel.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if err := parallel.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg parallel.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
...
func trainBatch(batch int, cfg parallel.Config) (x, y *core.Tensor[float32]) {
