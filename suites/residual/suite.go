...
func Cases() []Case {
	return []Case{
		{Name: "Forward smoke (Residual y=F(x)+x)", Run: forwardSmoke},
...
func RunAll() error {
	...
	if len(fails) > 0 {
		return fmt.Errorf("residual: %d failed: %s", len(fails), strings.Join(fails, ", "))
	}
	return nil
}
...
func RunOne(n int) error {
	cs := Cases()
	if n < 1 || n > len(cs) {
		return fmt.Errorf("residual: case %d out of range 1..%d", n, len(cs))
	}
	suites.BeginCase()
	fmt.Printf("  [%d] %s … ", n, cs[n-1].Name)
	if err := cs[n-1].Run(); err != nil {
		suites.EndCase("residual", cs[n-1].Name, "FAIL", err.Error())
		fmt.Printf("FAIL\n      %v\n", err)
		return err
	}
	suites.EndCase("residual", cs[n-1].Name, "PASS", "")
	fmt.Println("PASS")
	return nil
}
...
func newLayer(cfg residual.Config, dt core.DType, format quant.Format, be core.Backend) (*residual.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	l, err := residual.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, eyePacked(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg residual.Config, batch int) *core.Tensor[float32] {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = residual.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = residual.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	x := makeInput(cfg, 1)
	_, post, err := residual.Forward(l, x)
	if err != nil {
		return err
	}
	var sum float64
...
func backwardFiniteDiff() error {
	...
	_, dW, err := residual.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
...
func webGPUNoDevice() error {
	...
	_, _, err = residual.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func fullMatrixGaps() error {
	perms := residual.AllPermutations()
	var failN int
	var samples []string
	for _, p := range perms {
		var err error
