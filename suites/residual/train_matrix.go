...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Residual TRAIN — %s\n", title)
	fmt.Printf("  cell Residual dim=%d depth=%d T=%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Residual TRAIN — %s\n", title)
	fmt.Printf("  cell Residual dim=%d depth=%d T=%d batch=%d  cells=%d\n\n",
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg residual.Config) (ns int64, status, note string) {
	...
	g, err := buildResidualCube(n, be, dt, format, cfg)
	if err != nil {
...
func buildResidualCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg residual.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if err := residual.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg residual.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
...
func trainBatch(batch int, cfg residual.Config) (x, y *core.Tensor[float32]) {
