...
func rmsSimdScaleParity() error {
	...
	if max > 1e-4 {
		return fmt.Errorf("RMS SIMD scale maxΔ=%g", max)
	}
	fmt.Printf("(RMSNormScaleF32 Δ=%.3g) ", max)
	return nil
}

func rmsNormWebGPUBwdParity() error {
	if !webgpu.Available() {
	...
	run := func(be core.Backend) (*core.Tensor[float32], *core.Tensor[float32], error) {
		...
		gIn, gW, err := rmsnorm.Backward(l, g, x, pre)
		return gIn, gW, err
	}
	...
	if maxX > tol || maxW > tol {
		return fmt.Errorf("RMS WebGPU bwd dxΔ=%g dγΔ=%g", maxX, maxW)
	}
	fmt.Printf("(RMS bwd GPU dxΔ=%.3g dγΔ=%.3g) ", maxX, maxW)
	return nil
}
