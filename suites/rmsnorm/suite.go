...
func Cases() []Case {
	return []Case{
		{Name: "Forward smoke (γ≈1)", Run: forwardSmoke},
		{Name: "Seq layout [batch,seq,dim] smoke", Run: seqLayoutSmoke},
		{Name: "Backward finite-diff dγ spot-check", Run: backwardFiniteDiff},
...
func RunAll() error {
	...
	if len(fails) > 0 {
		return fmt.Errorf("rmsnorm: %d failed: %s", len(fails), strings.Join(fails, ", "))
	}
	return nil
}
...
func RunOne(n int) error {
	cs := Cases()
	if n < 1 || n > len(cs) {
		return fmt.Errorf("rmsnorm: case %d out of range 1..%d", n, len(cs))
	}
	suites.BeginCase()
	fmt.Printf("  [%d] %s … ", n, cs[n-1].Name)
	if err := cs[n-1].Run(); err != nil {
		suites.EndCase("rmsnorm", cs[n-1].Name, "FAIL", err.Error())
		fmt.Printf("FAIL\n      %v\n", err)
		return err
	}
	suites.EndCase("rmsnorm", cs[n-1].Name, "PASS", "")
	fmt.Println("PASS")
	return nil
}
...
func newLayer(cfg rmsnorm.Config, dt core.DType, format quant.Format, be core.Backend) (*rmsnorm.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	l, err := rmsnorm.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, onesInit(cfg.Dim))
	if err != nil {
		return nil, err
	}
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	pre, post, err := rmsnorm.Forward(l, x)
	if err != nil {
	...
	_, _, err = rmsnorm.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = rmsnorm.Forward(l, makeInput(2, tinyCfg().Dim))
	return err
}
...
func forwardSmoke() error {
	...
	_, post, err := rmsnorm.Forward(l, x)
	if err != nil {
		return err
	}
	var sum float64
...
func backwardFiniteDiff() error {
	...
	_, dW, err := rmsnorm.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
...
func gradVerifyBackends() error {
	...
	if max > 1e-2 {
		return fmt.Errorf("CPU↔SIMD dγ max err %v", max)
	}
...
func webGPUNoDevice() error {
	...
	_, _, err = rmsnorm.Forward(l, makeInput(1, tinyCfg().Dim))
	if err == nil {
