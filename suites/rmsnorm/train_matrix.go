...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  RMSNorm TRAIN — %s\n", title)
	fmt.Printf("  cell RMSNorm dim=%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  RMSNorm TRAIN — %s\n", title)
	fmt.Printf("  cell RMSNorm dim=%d batch=%d  cells=%d\n\n",
		cfg.Dim, batch, total)
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg rmsnorm.Config) (ns int64, status, note string) {
...
func buildRMSNormCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg rmsnorm.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if err := rmsnorm.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch, dim, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
	...
	st, nt := suites.StampBackendNote("rmsnorm", be == core.BackendSIMD, be == core.BackendWebGPU, "OK", "")
