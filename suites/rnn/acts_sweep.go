...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  RNN activation numeric sweep — %d Tensor[T] kinds × %d backends\n",
		len(allActKinds), len(backends))
	fmt.Printf("  weights=FormatNone Float32  in=%d hid=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		tinyCfg().InputSize, tinyCfg().HiddenSize, tinyCfg().SeqLen, simd.Enabled(), webgpu.Available())
...
func smokeAct[T core.Numeric](be core.Backend) error {
	...
	l, err := rnn.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, packed)
	if err != nil {
		return err
	}
	l.Exec.Backend = be
	l.Exec.MultiCore = false
	x := core.NewTensor[T](2, cfg.SeqLen, cfg.InputSize)
	for i := range x.Data {
