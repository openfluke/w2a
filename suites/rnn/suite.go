...
func Cases() []Case {
	return []Case{
		{Name: "Forward smoke (tanh RNN)", Run: forwardSmoke},
		{Name: "Seq layout [batch,seq,dim] smoke", Run: shapeSmoke},
...
func RunAll() error {
	...
	if len(fails) > 0 {
		return fmt.Errorf("rnn: %d failed: %s", len(fails), strings.Join(fails, ", "))
	}
	return nil
}
...
func RunOne(n int) error {
	cs := Cases()
	if n < 1 || n > len(cs) {
		return fmt.Errorf("rnn: case %d out of range 1..%d", n, len(cs))
	}
	suites.BeginCase()
	fmt.Printf("  [%d] %s … ", n, cs[n-1].Name)
	if err := cs[n-1].Run(); err != nil {
		suites.EndCase("rnn", cs[n-1].Name, "FAIL", err.Error())
		fmt.Printf("FAIL\n      %v\n", err)
		return err
	}
	suites.EndCase("rnn", cs[n-1].Name, "PASS", "")
	fmt.Println("PASS")
	return nil
}
...
func initPacked(cfg rnn.Config) []float32 {
	...
	// strengthen a few IH diagonal taps
	for h := 0; h < cfg.HiddenSize && h < cfg.InputSize; h++ {
		w[h*cfg.InputSize+h] = 0.5
	}
	return w
}

func newLayer(cfg rnn.Config, dt core.DType, format quant.Format, be core.Backend) (*rnn.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	l, err := rnn.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, initPacked(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg rnn.Config, batch int) *core.Tensor[float32] {
	x := core.NewTensor[float32](batch, cfg.SeqLen, cfg.InputSize)
	for i := range x.Data {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = rnn.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = rnn.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	fmt.Printf("(in=%d hid=%d T=%d) ", cfg.InputSize, cfg.HiddenSize, cfg.SeqLen)
	return nil
}

func shapeSmoke() error {
	cfg := tinyCfg()
	cfg.HiddenSize = 12 // non-preserving ok for unit smoke
	l, err := newLayer(cfg, core.DTypeFloat32, quant.FormatNone, core.BackendCPUTiled)
	if err != nil {
		return err
	}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := rnn.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
	master, ok := l.IH.Weights.MasterF32()
	if !ok || len(master) == 0 {
...
func webGPUNoDevice() error {
	...
	_, _, err = rnn.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func simdWebGPUAllQuants() error {
	...
	for _, f := range quant.AllFormats {
		...
		if f == quant.FormatAffinePacked && (!suites.AffinePackable(tinyCfg().HiddenSize, tinyCfg().InputSize) || !suites.AffinePackable(tinyCfg().HiddenSize, tinyCfg().HiddenSize)) {
