...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  RNN TRAIN — %s\n", title)
	fmt.Printf("  cell RNN in=%d hid=%d T=%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  RNN TRAIN — %s\n", title)
	fmt.Printf("  cell RNN in=%d hid=%d T=%d batch=%d  cells=%d\n\n",
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg rnn.Config) (ns int64, status, note string) {
	...
	g, err := buildRNNCube(n, be, dt, format, cfg)
	if err != nil {
...
func buildRNNCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg rnn.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if w, ok := l.IH.Weights.MasterF32(); ok && len(w) > 0 {
					w[0] += float32(z+y+x) * 0.001
				}
				if err := rnn.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg rnn.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
...
func trainBatch(batch int, cfg rnn.Config) (x, y *core.Tensor[float32]) {
