...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  Sequential activation numeric sweep — %d Tensor[T] kinds × %d backends\n",
		len(allActKinds), len(backends))
	fmt.Printf("  weights=FormatNone Float32  dim=%d depth=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		tinyCfg().Dim, tinyCfg().Depth, tinyCfg().SeqLen, simd.Enabled(), webgpu.Available())
...
func smokeAct[T core.Numeric](be core.Backend) error {
	cfg := tinyCfg()
	packed := make([]T, cfg.WeightCount())
	childN := cfg.Dim * cfg.Dim
	for d := 0; d < cfg.Depth; d++ {
		off := d * childN
		for i := 0; i < cfg.Dim; i++ {
			packed[off+i*cfg.Dim+i] = core.FromFloat64[T](1)
		}
	}
	l, err := sequential.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, packed)
	if err != nil {
		return err
	}
	l.Exec.Backend = be
	l.Exec.MultiCore = false
	x := core.NewTensor[T](2, cfg.SeqLen, cfg.Dim)
	for i := range x.Data {
