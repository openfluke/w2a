...
func TimedMatrix() error {
	...
	fmt.Printf("\n  Sequential timed matrix — FormatNone × %d dtypes × %d backends\n", len(core.AllDTypes), len(backends))
	fmt.Printf("  shape batch=%d dim=%d depth=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.Dim, cfg.Depth, cfg.SeqLen, simd.Enabled(), webgpu.Available())
...
func TimedQuantMatrix() error {
	...
	fmt.Printf("\n  Sequential timed quant matrix — %d formats × %d backends (Float32)\n", len(quant.AllFormats), len(backends))
	fmt.Printf("  shape batch=%d dim=%d depth=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.Dim, cfg.Depth, cfg.SeqLen, simd.Enabled(), webgpu.Available())
...
func timeCell(dt core.DType, format quant.Format, be core.Backend, cfg sequential.Config, batch, warm, iters int) (fwdNs, bwdNs int64, status, note string) {
	...
	gOut := core.NewTensor[float32](batch, cfg.SeqLen, cfg.Dim)
	for i := range gOut.Data {
		gOut.Data[i] = 1
	}
	var pre *core.Tensor[float32]
	for i := 0; i < warm; i++ {
		pre, _, err = sequential.Forward(l, x)
		if err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
		if _, _, err = sequential.Backward(l, gOut, x, pre); err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
	}
	if iters < 1 {
