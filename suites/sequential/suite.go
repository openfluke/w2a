...
func Cases() []Case {
	return []Case{
		{Name: "Forward smoke (Dense→Dense Sequential)", Run: forwardSmoke},
		...
		{Name: "MIXED — Dense→RMSNorm→SwiGLU fwd+bwd+SGD", Run: mixedNonDenseChildren},
	}
}
...
func RunAll() error {
	...
	if len(fails) > 0 {
		return fmt.Errorf("sequential: %d failed: %s", len(fails), strings.Join(fails, ", "))
	}
	return nil
}
...
func RunOne(n int) error {
	cs := Cases()
	if n < 1 || n > len(cs) {
		return fmt.Errorf("sequential: case %d out of range 1..%d", n, len(cs))
	}
	suites.BeginCase()
	fmt.Printf("  [%d] %s … ", n, cs[n-1].Name)
	if err := cs[n-1].Run(); err != nil {
		suites.EndCase("sequential", cs[n-1].Name, "FAIL", err.Error())
		fmt.Printf("FAIL\n      %v\n", err)
		return err
	}
	suites.EndCase("sequential", cs[n-1].Name, "PASS", "")
	fmt.Println("PASS")
	return nil
}
...
func newLayer(cfg sequential.Config, dt core.DType, format quant.Format, be core.Backend) (*sequential.Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	l, err := sequential.NewConfigured(cfg, core.DTypeFloat32, quant.FormatNone, eyePacked(cfg))
	if err != nil {
		return nil, err
	}
...
func makeInput(cfg sequential.Config, batch int) *core.Tensor[float32] {
...
func smoke(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = sequential.Backward(l, g, x, pre)
	if err != nil {
...
func smokeForwardOnly(dt core.DType, format quant.Format, be core.Backend) error {
	...
	_, _, err = sequential.Forward(l, makeInput(cfg, 2))
	return err
}
...
func forwardSmoke() error {
	...
	x := makeInput(cfg, 1)
	_, post, err := sequential.Forward(l, x)
	if err != nil {
		return err
	}
	var sum float64
	...
	fmt.Printf("(dim=%d depth=%d T=%d) ", cfg.Dim, cfg.Depth, cfg.SeqLen)
	return nil
}
...
func backwardFiniteDiff() error {
	...
	_, dW, err := sequential.Backward(l, g, x, pre)
	if err != nil {
		return err
	}
	master, ok := l.Children[0].Weights.MasterF32()
	if !ok || len(master) == 0 {
	...
	lossAt := func() (float64, error) {
		_, p, err := sequential.Forward(l, x)
		if err != nil {
			return 0, err
		}
		var loss float64
...
func webGPUNoDevice() error {
	...
	_, _, err = sequential.Forward(l, makeInput(tinyCfg(), 1))
	if err == nil {
...
func simdWebGPUAllQuants() error {
	...
	for _, f := range quant.AllFormats {
		...
		if f == quant.FormatAffinePacked && (!suites.AffinePackable(tinyCfg().Dim, tinyCfg().Dim)) {
...
func fullMatrixGaps() error {
	perms := sequential.AllPermutations()
	var failN int
	var samples []string
	for _, p := range perms {
		var err error
