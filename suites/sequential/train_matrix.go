...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Sequential TRAIN — %s\n", title)
	fmt.Printf("  cell Sequential dim=%d depth=%d T=%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
		cfg.Dim, cfg.Depth, cfg.SeqLen, batch, total, simd.Enabled(), webgpu.Available())
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  Sequential TRAIN — %s\n", title)
	fmt.Printf("  cell Sequential dim=%d depth=%d T=%d batch=%d  cells=%d\n\n",
		cfg.Dim, cfg.Depth, cfg.SeqLen, batch, total)
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg sequential.Config) (ns int64, status, note string) {
	...
	g, err := buildSequentialCube(n, be, dt, format, cfg)
	if err != nil {
...
func buildSequentialCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg sequential.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if w, ok := l.Children[0].Weights.MasterF32(); ok && len(w) > 0 {
					w[0] += float32(z+y+x) * 0.001
				}
				if err := sequential.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch int, cfg sequential.Config, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
...
func trainBatch(batch int, cfg sequential.Config) (x, y *core.Tensor[float32]) {
