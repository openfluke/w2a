...
func ActNumericSweep() error {
	...
	fmt.Printf("\n  Softmax activation numeric sweep — %d Tensor[T] kinds × %d backends\n",
		len(allActKinds), len(backends))
	fmt.Printf("  weightless  dim=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		tinyCfg().Dim, tinyCfg().SeqLen, simd.Enabled(), webgpu.Available())
