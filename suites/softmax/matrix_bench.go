...
func TimedMatrix() error {
	...
	fmt.Printf("\n  Softmax timed matrix — FormatNone × %d dtypes × %d backends (weightless ALU)\n", len(core.AllDTypes), len(backends))
	fmt.Printf("  shape batch=%d dim=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.Dim, cfg.SeqLen, simd.Enabled(), webgpu.Available())
...
func TimedQuantMatrix() error {
	...
	fmt.Printf("\n  Softmax timed quant matrix — %d formats × %d backends (weightless)\n", len(quant.AllFormats), len(backends))
	fmt.Printf("  shape batch=%d dim=%d T=%d  SIMD=%v WebGPU=%v\n\n",
		batch, cfg.Dim, cfg.SeqLen, simd.Enabled(), webgpu.Available())
...
func timeCell(dt core.DType, format quant.Format, be core.Backend, cfg softmax.Config, batch, warm, iters int) (fwdNs, bwdNs int64, status, note string) {
	...
	for i := 0; i < warm; i++ {
		pre, _, err = softmax.Forward(l, x)
		if err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
		if _, _, err = softmax.Backward(l, gOut, x, pre); err != nil {
			return 0, 0, failOrGap(be), err.Error()
		}
	}
	if iters < 1 {
