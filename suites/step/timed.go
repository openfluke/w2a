...
// TimedDenseFormatNone — Dense StepMesh CPU vs SIMD across all 34 dtypes.
func TimedDenseFormatNone() error {
	return timedDenseCompare(
		"FormatNone × all 34 dtypes (Dense %d→%d StepMesh)",
...
// TimedDenseQuants — Dense StepMesh CPU vs SIMD across all quants (Float32).
func TimedDenseQuants() error {
	return timedDenseCompare(
		"all quants × Float32 (Dense %d→%d StepMesh)",
		func(yield func(dt core.DType, format quant.Format, label string)) {
...
// TimedLayersCPUVsSIMD — every layer FormatNone Float32, CPU vs SIMD.
func TimedLayersCPUVsSIMD() error {
	kinds := polyops.AllKinds()
	fmt.Printf("\n  Step TIMED layers — FormatNone Float32 × %d layers × CPU vs SIMD\n", len(kinds))
...
func timedDenseCompare(titleFmt string, each func(func(dt core.DType, format quant.Format, label string))) error {
	fmt.Printf("\n  Step TIMED Dense CPU vs SIMD — "+titleFmt+"\n", timedDim, timedDim)
	fmt.Printf("  warm=%d iters=%d  SIMD=%v\n\n", timedWarm, timedIters, simd.Enabled())
...
func timeDenseStep(dt core.DType, format quant.Format, be core.Backend) (ns int64, status, note string) {
	...
	return benchStepMesh(g, x, target, be)
}
...
func benchStepMesh(g *architecture.Grid, x, target *core.Tensor[float32], be core.Backend) (ns int64, status, note string) {
	for i := 0; i < timedWarm; i++ {
		if _, _, err := training.StepMesh(g, x, target, 1, timedLR); err != nil {
...
func recTimed(op, dt, format, backend, status, note string) {
	suites.RecordCell(suites.Cell{
		Layer: "step", Op: op, DType: dt, Format: format, Backend: backend,
