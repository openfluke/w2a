...
func timedTrainFormatNoneGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  SwiGLU TRAIN — %s\n", title)
	fmt.Printf("  cell SwiGLU in=%d inter=%d batch=%d  cells=%d  SIMD=%v WebGPU=%v\n\n",
		cfg.InputDim, cfg.IntermediateDim, batch, total, simd.Enabled(), webgpu.Available())
...
func timedTrainQuantGrids(sizes []int, title string) error {
	...
	fmt.Printf("\n  SwiGLU TRAIN — %s\n", title)
	fmt.Printf("  cell SwiGLU in=%d inter=%d batch=%d  cells=%d\n\n",
		cfg.InputDim, cfg.IntermediateDim, batch, total)
...
func timeTrainCube(n int, be core.Backend, batch, warm, iters int, lr float64, dt core.DType, format quant.Format, cfg swiglu.Config) (ns int64, status, note string) {
	...
	g, err := buildSwiGLUCube(n, be, dt, format, cfg)
	if err != nil {
		return 0, failOrGap(be), err.Error()
	}
	return benchTrainStep(g, batch, cfg.InputDim, warm, iters, lr, be)
}

func buildSwiGLUCube(n int, be core.Backend, dt core.DType, format quant.Format, cfg swiglu.Config) (*architecture.Grid, error) {
	...
	for z := 0; z < n; z++ {
		for y := 0; y < n; y++ {
			for x := 0; x < n; x++ {
				...
				if w, ok := l.Gate.Weights.MasterF32(); ok && len(w) > 0 {
					w[0] += float32(z+y+x) * 0.001
				}
				if err := swiglu.Place(g, z, y, x, 0, l); err != nil {
					return nil, err
				}
			}
		}
...
func benchTrainStep(g *architecture.Grid, batch, dim, warm, iters int, lr float64, be core.Backend) (ns int64, status, note string) {
	x, y := trainBatch(batch, dim)
	for i := 0; i < warm; i++ {
...
func trainBatch(batch, dim int) (x, y *core.Tensor[float32]) {
	...
	y = core.NewTensor[float32](batch, dim)
	for i := range x.Data {
