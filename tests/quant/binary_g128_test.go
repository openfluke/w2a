...
func TestMatVecBinaryG128Exact(t *testing.T) {
	...
	wordsPerRow := cols / 32
	groupsPerRow := cols / quant.BinaryG128Group
	...
	b, err := quant.BlobFromMLX1Bit(weight, scales, biases, rows, cols)
	if err != nil {
		t.Fatal(err)
	}
	x := make([]float32, cols)
	...
	if err := quant.MatVec(b, x, got); err != nil {
		t.Fatal(err)
	}
	row := make([]float32, cols)
	for r := 0; r < rows; r++ {
		if err := quant.DecodeRow(b, r, row); err != nil {
			t.Fatal(err)
		}
		var want float32
		for c := 0; c < cols; c++ {
...
func BenchmarkMatVecBinaryG128(b *testing.B) {
	...
	blob, err := quant.BlobFromMLX1Bit(weight, scales, biases, rows, cols)
	if err != nil {
		b.Fatal(err)
	}
	x := make([]float32, cols)
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if err := quant.MatVec(blob, x, y); err != nil {
			b.Fatal(err)
		}
	}
}
