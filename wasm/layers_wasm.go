...
// placeSpec is the common JSON body for place* methods:
// { "z":0,"y":0,"x":0,"l":0, "in":4,"out":4, "act":"relu", "dtype":1, "format":0, ...cfg }
...
func parsePlace(args []js.Value) (placeSpec, []byte, error) {
	if len(args) < 1 {
		return placeSpec{}, nil, fmt.Errorf("place needs JSON config")
	}
...
func registerLayerGlobals() {
	js.Global().Set("listWelvetLayerTypes", js.FuncOf(func(this js.Value, args []js.Value) interface{} {
	...
	js.Global().Set("listWelvetDTypes", js.FuncOf(func(this js.Value, args []js.Value) interface{} {
	...
	js.Global().Set("listWelvetFormats", js.FuncOf(func(this js.Value, args []js.Value) interface{} {
	...
	js.Global().Set("listWelvetBackends", js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		return jsonStr([]map[string]any{
			{"id": int(core.BackendCPUTiled), "name": core.BackendCPUTiled.String()},
			{"id": int(core.BackendSIMD), "name": core.BackendSIMD.String()},
			{"id": int(core.BackendWebGPU), "name": core.BackendWebGPU.String()},
		})
	}))
	js.Global().Set("welvetPermutationOK", js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		// welvetPermutationOK(kind, dtype, format, backend) — dense matrix gate (most layers share it)
		if len(args) < 4 {
			return false
		}
		dt := core.DType(args[1].Int())
		format := quant.Format(args[2].Int())
		be := core.Backend(args[3].Int())
		kind := strings.ToLower(args[0].String())
		switch kind {
		case "gdn":
			return gdn.PermutationOK(dt, format, be)
		default:
			return dense.PermutationOK(dt, format, be)
		}
	}))
	js.Global().Set("listWelvetSuiteCatalog", js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		// Mirrors apps/w2a/suites + portable WASM ports for parity audits.
		return jsonStr([]string{
...
func placeDenseOnGrid(g *architecture.Grid, args []js.Value) interface{} {
	s, _, err := parsePlace(args)
	if err != nil {
	...
	if in <= 0 {
		return errObj("dense needs in/out or dim")
	}
	layer, err := dense.NewConfigured[float32](in, out, actOf(s), dtOf(s), fmtOf(s), nil)
	if err != nil {
		return errObj(err.Error())
	}
	if err := dense.Place(g, s.Z, s.Y, s.X, s.L, layer); err != nil {
		return errObj(err.Error())
	}
	return okObj()
}

func placeTypedOnGrid(g *architecture.Grid, kind string, args []js.Value) interface{} {
	s, raw, err := parsePlace(args)
	if err != nil {
		return errObj(err.Error())
	}
	...
	default:
		return errObj("unknown layer kind " + kind)
	}
	return okObj()
}
